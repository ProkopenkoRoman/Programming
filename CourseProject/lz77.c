...
struct Code Find( char* dict, char *buff , int M, int N ) {
	...
	for( i = 0; i < N - 1 ; i++) 
	{
		...
		if ( num >= buff ) 
		{
			...
			// запишем её код
			if ( tmp ==  0 ) {
				// последнгий символ за подстрокой
				code.c = pstr[i];
				// длина и смещение в словаре в 0
				code.len = 0; code.num = 0;
				// освободить память
				free(pstr);
				// вернуть код
				return code;
	...
	// если цикл завершен
	// то вся подстрока длиной буфера - 1 сивмол найдена в словаре  
	if ( tmp == num && i == N - 1 ) {
		// запишем длину
		code.len = N - 1;
		// последний символ в буфеое
...
void Encode( FILE *fin, FILE *fout, int M, int N) {
	...
	char *input;	// входные и выходной массив
	char *output;
	...
	// найти длину выходного файлв в байтах
	fosize = offset / 8;
